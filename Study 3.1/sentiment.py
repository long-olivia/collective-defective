            ...
            for i in range(1,21):
                ...
                for j in range(0,len(all_text)):
                    ...
                    if j%4 == 0:
                        ...
                        c_score[i-1]=c_score[i-1]+call(all_text[j])
                    else:
                        d_score[i-1]=d_score[i-1]+call(all_text[j])
                a_score[i-1]=a_score[i-1]/(len(all_text)/2)
                b_score[i-1]=b_score[i-1]/(len(all_text)/2)
                c_score[i-1]=c_score[i-1]/(len(all_text)/2)
                d_score[i-1]=d_score[i-1]/(len(all_text)/2)
        output={
            "gpt": a_score,
            "claude": b_score,
            "llama": c_score,
