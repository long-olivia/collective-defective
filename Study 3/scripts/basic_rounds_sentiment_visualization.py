...
def plot(prompt_pair, arr):
    ...
    plt.title(f"Collective-Defective Score, GPT-4o {prompt_pair}, Name Condition")
    gpt_bars=plt.bar(x-1.5*width, gpt, width=width, color='powderblue', label='GPT-4o')
    claude_bars=plt.bar(x-0.5*width, claude, width=width, color='teal', label='GPT-4o')
    llama_bars=plt.bar(x+0.5*width, llama, width=width, color='lightcoral', label='GPT-4o')
    qwen_bars=plt.bar(x+1.5*width, qwen, width=width, color='firebrick', label='GPT-4o')
    ...
    plt.ylim(top=1.2)
    plt.tight_layout()
    plt.savefig(f"Self_{prompt_pair}_gpt")

def prepare(prompt_pair):
    file=open("self_openai_agg.json")
    data=json.load(file)
