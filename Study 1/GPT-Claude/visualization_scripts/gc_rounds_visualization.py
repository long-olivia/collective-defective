...
def plot_basic(prompt_pair, arr, llama_err, qwen_err):
    ...
    llama_bars=plt.bar(np.arange(len(llama)), llama, width=width, yerr=llama_err, capsize=5, color='powderblue', label='GPT-4o')
...
def prepare(prompt_pair):
    ...
    plot_basic(labels[prompt_pair], b_arr, b_err[0], b_err[1])
    plot_discrim(labels[prompt_pair], s_arr, s_err[0], s_err[1])
...
if __name__ == "__main__":
    ...
    load("basic_round.json", "gc_basic_rounds_SE.json", "discrim_round.json", "gc_self_rounds_SE.json")
