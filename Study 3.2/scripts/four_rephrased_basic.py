...
def run(a_prompt, b_prompt, c_prompt, d_prompt, model):
    ...
    sys_c=initialize(c_prompt)
    sys_d=initialize(d_prompt)
...
if __name__ == "__main__":
    if not OPENROUTER_API_KEY:
        ...
        if len(sys.argv) < 5:
            print("Usage: python rephrased_basic.py <a_prompt> <b_prompt> <c_prompt> <d_prompt> <model>")
        else:
            a_prompt=sys.argv[1]
            b_prompt=sys.argv[2]
            c_prompt=sys.argv[3]
            ...
            output_filename = f"{model}_results/basic_{a_prompt}_{timestamp}.json"
