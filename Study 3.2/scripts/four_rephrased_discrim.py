...
def run(a_prompt, b_prompt, c_prompt, d_prompt, model):
...
if __name__ == "__main__":
    if not OPENROUTER_API_KEY:
        ...
        if len(sys.argv) < 5:
            print("Usage: python rephrased_discrim.py <a_prompt> <b_prompt> <c_prompt> <d_prompt> <model>")
        else:
            a_prompt=sys.argv[1]
            b_prompt=sys.argv[2]
            c_prompt=sys.argv[3]
            d_prompt=sys.argv[4]
            model=sys.argv[5]
            run(a_prompt, b_prompt, c_prompt, d_prompt, model)
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_filename = f"self_{model}_results/discrim_{a_prompt}_{timestamp}.json"
            os.makedirs(f"self_{model}_results", exist_ok=True)
            try:
